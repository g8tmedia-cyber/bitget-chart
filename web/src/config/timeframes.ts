 ...
 *
 * Order matches the reference layout (seconds → minutes → hours →
 * days → weeks → months), so the chart's top toolbar reads naturally
 * left-to-right. The last entry (`1M`) gets a dropdown caret in the
 * UI to suggest a longer list behind it.
 *
 ...
 * - `seconds` is the candle duration in seconds.
 */
...
export const TIMEFRAMES: Timeframe[] = [
  { label: "1s", granularity: "1s", wsChannel: "candle1s", seconds: 1 },
